...
export const useArrayState = <T>(initialArray: T[] = []) => {
  ...
  const operations = useMemo(() => ({
    ...
    update: (index: number, item: T) => setArray(prev => 
      prev.map((existingItem, i) => i === index ? item : existingItem)
    ),
    clear: () => setArray([]),
    replace: (newArray: T[]) => setArray(newArray),
    toggle: (item: T, keySelector?: (item: T) => any) => setArray(prev => {
      ...
      return exists 
        ? prev.filter(existingItem => keySelector 
...
export const useMapState = <K, V>(initialMap?: Map<K, V>) => {
  ...
  const operations = useMemo(() => ({
    ...
    update: (key: K, updater: (value: V | undefined) => V) => setMap(prev => {
      const newMap = new Map(prev);
