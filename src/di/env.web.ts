...
import { AppConfig, EnvironmentVariables, EnvProvider } from '../types';
 ...
 */
export const createWebAppConfig = (
  envProvider: EnvProvider = new WebEnvProvider()
): AppConfig => {
  return {
    wildDuckApiToken: envProvider.get('VITE_WILDDUCK_API_TOKEN', ''),
    wildDuckBackendUrl: envProvider.get(
      'VITE_WILDDUCK_BACKEND_URL',
      'http://localhost:8080'
    ),
    indexerBackendUrl: envProvider.get(
      'VITE_INDEXER_BACKEND_URL',
      'https://indexer.0xmail.box'
    ),
    revenueCatApiKey: envProvider.get('VITE_REVENUECAT_API_KEY', ''),
    walletConnectProjectId: envProvider.get(
      'VITE_WALLETCONNECT_PROJECT_ID',
      '2f05ae7f1f5344b5ae075017bfaaa560'
    ),
    ...
    firebase: {
      ...
      messagingSenderId: envProvider.get(
    ...
    useCloudflareWorker:
      envProvider.get('VITE_USE_CLOUDFLARE_WORKER', 'false') === 'true',
    cloudflareWorkerUrl: envProvider.get(
      'VITE_CLOUDFLARE_WORKER_URL',
      'https://wildduck-proxy.workers.dev'
    ),
    useMockFallback:
      envProvider.get('VITE_USE_MOCK_FALLBACK', 'true') === 'true',
  };
};
