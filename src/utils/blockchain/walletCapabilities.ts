...
export function getWalletCapabilities(walletName: string): WalletCapability {
  ...
  if (name.includes('metamask') || name.includes('walletconnect') || 
      name.includes('coinbase') || name.includes('injected') ||
 ...
 */
export function isWalletAvailable(walletName: string, chainType: 'evm' | 'solana'): boolean {
