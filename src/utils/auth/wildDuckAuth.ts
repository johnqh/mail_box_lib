 ...
 */
const storeAuthData = (username: string, userId: string, token?: string): void => {
  try {
    ...
    console.log('✅ Stored WildDuck auth data for:', username, 'User ID:', userId);
  } catch (error) {
  ...
  try {
    ...
    if (cachedData) {
      ...
      if (cacheAge < ONE_HOUR && parsed.userId) {
        console.log('📦 Using cached WildDuck auth for:', username, 'User ID:', parsed.userId);
        return {
          success: true,
...
export const getWildDuckUserId = async (
  ...
  if (!options?.forceReauth) {
    ...
    if (storedUserId) {
      console.log('📋 Found stored WildDuck user ID:', storedUserId, 'for:', normalizedUsername);
      return storedUserId;
    }
  }
  ...
  if (options?.signMessage && options?.chainType) {
    try {
      console.log('🔐 Attempting WildDuck authentication for:', normalizedUsername);
      ...
      const formattedSignature = formatSignatureForWildDuck(signature, options.chainType);
      
      console.log('📝 Signature (formatted):', formattedSignature.substring(0, 20) + '...');
      ...
      if (authResponse.success && authResponse.id) {
        ...
        console.log('✅ WildDuck authentication successful! User ID:', authResponse.id);
  ...
  console.warn('ℹ️ To fix this, ensure proper authentication with WildDuck API');
