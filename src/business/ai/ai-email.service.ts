...
export interface AIEmailService {
  ...
  summarizeEmail(email: Email, options?: AIProcessingOptions): Promise<EmailSummary>;
  extractEntities(email: Email, options?: AIProcessingOptions): Promise<EmailEntities>;
...
class AIEmailServiceImpl implements AIEmailService {
  ...
  async categorizeEmail(email: Email, options?: AIProcessingOptions): Promise<EmailCategory> {
    try {
      ...
      if (hasWeb3Content.score > 0.7) {
        return {
          ...
          reasoning: 'Contains Web3-related content such as wallet addresses, transaction hashes, or DeFi protocols'
        };
      }
      ...
      if (priorityScore > 0.8) {
        return {
          ...
          reasoning: 'High priority based on sender, keywords, and urgency indicators'
        };
      }
      ...
      return {
        primary: 'personal',
        confidence: 0.5,
        reasoning: 'Default categorization - no strong indicators for other categories'
      };
  ...
  async summarizeEmail(email: Email, options?: AIProcessingOptions): Promise<EmailSummary> {
    try {
      ...
      if (content.length > 200) {
        ...
        summary = sentences[0]?.trim() + (keyPoints.length > 1 ? ` Key topics: ${keyPoints.slice(1).join(', ')}` : '');
      } else {
  ...
  async extractEntities(email: Email, options?: AIProcessingOptions): Promise<EmailEntities> {
    try {
  ...
  async detectSpam(email: Email): Promise<{ isSpam: boolean; confidence: number; reasons: string[] }> {
    const reasons: string[] = [];
    let spamScore = 0;

    // Check subject line
    const subject = email.subject.toLowerCase();
    const spamKeywords = ['urgent', 'winner', 'congratulations', 'free', 'limited time', 'act now', '!!!'];
    const subjectSpamCount = spamKeywords.filter(keyword => subject.includes(keyword)).length;
    ...
    const hasMultipleAddresses = (email.body.match(this.WEB3_ADDRESS_REGEX) || []).length > 3;
  ...
  async suggestLabels(email: Email): Promise<{ label: string; confidence: number }[]> {
    const suggestions: { label: string; confidence: number }[] = [];
    const content = `${email.subject} ${email.body}`.toLowerCase();

    // Web3 labels
    if (content.includes('defi') || content.includes('swap') || content.includes('liquidity')) {
      suggestions.push({ label: 'DeFi', confidence: 0.8 });
    }
    if (content.includes('nft') || content.includes('opensea') || content.includes('mint')) {
      suggestions.push({ label: 'NFT', confidence: 0.8 });
    }
    if (content.includes('dao') || content.includes('governance') || content.includes('proposal')) {
  ...
  private detectWeb3Content(content: string): { score: number; categories: string[] } {
    let score = 0;
    ...
    const web3Keywords = ['defi', 'nft', 'dao', 'blockchain', 'ethereum', 'solana', 'metamask', 'opensea', 'uniswap'];
    const matchedKeywords = web3Keywords.filter(keyword => content.includes(keyword));
  ...
  private detectFinancialContent(content: string): number {
    let score = 0;
    const financialKeywords = ['payment', 'invoice', 'transaction', 'money', 'USD', 'ETH', 'BTC', '$', '€', '£'];
    const matchedKeywords = financialKeywords.filter(keyword => content.includes(keyword.toLowerCase()));
  ...
  private calculatePriorityScore(email: Email): number {
    ...
    const priorityKeywords = ['urgent', 'asap', 'important', 'critical', 'deadline', 'emergency'];
    const subject = email.subject.toLowerCase();
    const matchedKeywords = priorityKeywords.filter(keyword => subject.includes(keyword));
    score += matchedKeywords.length * 0.2;

    // Check for known important senders (simplified)
    if (email.from.includes('@ethereum.org') || email.from.includes('security') || email.from.includes('admin')) {
      score += 0.3;
    }

    // Check for deadline mentions
    if (email.body.toLowerCase().includes('deadline') || email.body.toLowerCase().includes('due date')) {
      score += 0.2;
    }
  ...
  private detectSocialContent(content: string, from: string): number {
    let score = 0;
    const socialKeywords = ['hello', 'hi', 'thanks', 'thank you', 'how are you', 'hope you', 'personal', 'family'];
    const matchedKeywords = socialKeywords.filter(keyword => content.includes(keyword));
    score = matchedKeywords.length * 0.1;
  ...
  private detectPromotionalContent(content: string): number {
    let score = 0;
    const promoKeywords = ['sale', 'discount', 'offer', 'deal', 'promotion', 'limited time', 'subscribe', 'unsubscribe'];
    const matchedKeywords = promoKeywords.filter(keyword => content.includes(keyword));
    score = matchedKeywords.length * 0.15;

    // Check for marketing phrases
    if (content.includes('click here') || content.includes('call to action') || content.includes('learn more')) {
      score += 0.2;
    }
  ...
  private analyzeSentimentBasic(text: string): 'positive' | 'negative' | 'neutral' {
    const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'thank', 'pleased', 'happy'];
    const negativeWords = ['bad', 'terrible', 'awful', 'problem', 'issue', 'error', 'failed', 'wrong', 'disappointed'];
  ...
  private detectUrgency(email: Email): 'low' | 'medium' | 'high' | 'urgent' {
    ...
    const highKeywords = ['important', 'priority', 'deadline', 'time sensitive'];
  ...
  private detectActionRequired(content: string): boolean {
    const actionKeywords = ['please', 'action required', 'respond', 'reply', 'confirm', 'approve', 'review', 'sign', 'complete'];
    return actionKeywords.some(keyword => content.toLowerCase().includes(keyword));
  }
  ...
  private extractAmounts(content: string): { value: number; currency: string; context?: string }[] {
  ...
  private extractPeople(content: string, fromEmail: string): { name: string; email?: string; role?: string }[] {
  ...
  private extractOrganizations(content: string): string[] {
    ...
    const orgKeywords = ['company', 'corporation', 'inc', 'llc', 'foundation', 'protocol', 'exchange'];
  ...
  private extractTopics(content: string): string[] {
    const topics: string[] = [];
    const web3Topics = ['DeFi', 'NFT', 'DAO', 'Staking', 'Trading', 'Governance', 'Yield Farming'];
    const generalTopics = ['Meeting', 'Project', 'Update', 'Invoice', 'Proposal', 'Agreement'];
