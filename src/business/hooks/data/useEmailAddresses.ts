...
export const useEmailAddresses = (): UseEmailAddressesReturn => {
   ...
   */
  const createWalletEmailAddress = useCallback((walletAddress: string): EmailAddress => {
    return {
      id: '1',
   ...
   */
  const createSNSEmailAddresses = useCallback(async (walletAddress: string, startId: number): Promise<EmailAddress[]> => {
   ...
   */
  const createENSEmailAddresses = useCallback(async (walletAddress: string, startId: number): Promise<EmailAddress[]> => {
    ...
    try {
      ...
      console.log(`📧 Total ENS email addresses created: ${ensEmailAddresses.length}`);
    } catch (error) {
   ...
   */
  const generateEmailAddresses = useCallback(async (
    ...
    if (chainType === ChainType.SOLANA) {
      ...
      fetchPromises.push(
        createSNSEmailAddresses(walletAddress, idCounter).catch(error => {
          errors.push(`SNS: ${error instanceof Error ? error.message : 'Unknown error'}`);
      ...
      fetchPromises.push(
        createENSEmailAddresses(walletAddress, idCounter).catch(error => {
          errors.push(`ENS: ${error instanceof Error ? error.message : 'Unknown error'}`);
          return []; // Return empty array on error to continue processing
        })
      );
      ...
      fetchPromises.push(
        ...
        createSNSEmailAddresses(walletAddress, idCounter + 100).catch(error => {
    ...
    console.log(`Total email addresses generated: ${allEmailAddresses.length}`);
   ...
   * Returns wallet address immediately, then enhances with ENS/SNS names
   */
  const refreshEmailAddresses = useCallback(async (walletAddress: string, chainType?: ChainType): Promise<EmailAddress[]> => {
    if (!walletAddress) {
      console.warn('Cannot refresh email addresses: no wallet address provided');
      setEmailAddresses([]);
    ...
    try {
      ...
      const progressCallback = (addresses: EmailAddress[]) => {
        ...
        console.log(`Progressive update: ${addresses.length} email addresses available`);
      };
      
      const newEmailAddresses = await generateEmailAddresses(walletAddress, detectedChainType, progressCallback);
      
      // Final update with all addresses
      setEmailAddresses(newEmailAddresses);
      setError(null);
      
      console.log('Email addresses successfully updated:', newEmailAddresses.length);
      return newEmailAddresses;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate email addresses';
