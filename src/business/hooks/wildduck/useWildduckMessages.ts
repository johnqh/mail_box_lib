...
const useWildduckMessages = (config: WildDuckConfig, _devMode: boolean = false): UseWildduckMessagesReturn => {
  ...
  const getMessage = async (
    ...
    try {
      ...
      const messageData = response.data as GetMessageResponse;

      // Update cache
      queryClient.setQueryData(['wildduck-message', userId, messageId], messageData);
  ...
  const sendMutation = useMutation({
    mutationKey: ['wildduck-send-message', config.cloudflareWorkerUrl || config.backendUrl],
    mutationFn: async ({
      userId,
      params,
    }: {
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['wildduck-messages', variables.userId] });
    },
  });

  // Update message mutation
  const updateMutation = useMutation({
    mutationKey: ['wildduck-update-message', config.cloudflareWorkerUrl || config.backendUrl],
    mutationFn: async ({
      userId,
      messageId,
      params,
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['wildduck-message', variables.userId, variables.messageId] });
      queryClient.invalidateQueries({ queryKey: ['wildduck-messages', variables.userId] });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationKey: ['wildduck-delete-message', config.cloudflareWorkerUrl || config.backendUrl],
    mutationFn: async ({
      userId,
      messageId,
    }: {
  ...
  const moveMutation = useMutation({
    mutationKey: ['wildduck-move-message', config.cloudflareWorkerUrl || config.backendUrl],
    mutationFn: async ({
      userId,
      messageId,
      targetMailbox,
  ...
  const isLoading = sendMutation.isPending || updateMutation.isPending || deleteMutation.isPending || moveMutation.isPending;
  ...
  return {
    ...
    // Update mutation
    updateMessage: async (userId: string, messageId: string, params: UpdateMessageParams) =>
    ...
    // Move mutation
    moveMessage: async (userId: string, messageId: string, targetMailbox: string) =>
