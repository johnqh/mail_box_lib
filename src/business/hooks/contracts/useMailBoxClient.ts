...
import {
  ...
  type DomainResult,
  type MessageResult,
  UnifiedMailBoxClient,
  type UnifiedTransaction,
  type UnifiedWallet,
} from '@johnqh/mail_box_contracts';

interface UseMailBoxClientOptions {
  wallet?: UnifiedWallet;
  config?: ChainConfig;
}

interface UseMailBoxClientReturn {
  // Client instance
  client: UnifiedMailBoxClient | null;
 ...
 */
export const useMailBoxClient = (
  options: UseMailBoxClientOptions = {}
): UseMailBoxClientReturn => {
  const [client, setClient] = useState<UnifiedMailBoxClient | null>(() => {
    if (options.wallet && options.config) {
      try {
        return new UnifiedMailBoxClient(options.wallet, options.config);
      } catch (error) {
        console.warn('Failed to initialize UnifiedMailBoxClient:', error);
        return null;
      }
    }
    return null;
  ...
  const initializeClient = useCallback(
    (wallet: UnifiedWallet, config: ChainConfig) => {
      try {
        const newClient = new UnifiedMailBoxClient(wallet, config);
        setClient(newClient);
        setError(null);
      } catch (err) {
        const errorMessage =
...
export type { UseMailBoxClientReturn, UseMailBoxClientOptions };
